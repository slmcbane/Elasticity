...
double manufactured_ux(double x, double y) {
    return (-3.0 / 2.0 * pow(x, 3) + (23.0 / 4.0) * pow(x, 2) - 5 * x) *
               sin(M_PI * y) +
           ((3.0 / 2.0) * pow(x, 3) - 13.0 / 4.0 * pow(x, 2) + 1) *
               cos(M_PI * y);
}

double manufactured_uy(double x, double y) {
...
double traction_lower_x(double x, [[maybe_unused]] double y) noexcept {
    return (5.0 / 208.0) * (M_PI * x * (6 * pow(x, 2) - 23 * x + 20) - 4) *
           (sin(M_PI * x) + 4);
}

double traction_lower_y(double x, [[maybe_unused]] double y) noexcept {
    return (5.0 / 364.0) * (sin(M_PI * x) + 4) *
           (-27 * pow(x, 2) + 79 * x - 40);
}
...
double forcing_x(double x, double y) noexcept {
    return (5.0 / 52.0) *
               ((13 - 18 * x) * cos(M_PI * y) + (18 * x - 23) * sin(M_PI * y)) *
               (sin(M_PI * x) * cos(M_PI * y) + 4) +
           (15.0 / 364.0) * (sin(M_PI * x) * cos(M_PI * y) + 4) *
               ((13 - 18 * x) * cos(M_PI * y) + (18 * x - 23) * sin(M_PI * y) +
                4) +
           (5.0 / 208.0) * (sin(M_PI * x) * cos(M_PI * y) + 4) *
               (-pow(M_PI, 2) * x * (6 * pow(x, 2) - 23 * x + 20) *
                    sin(M_PI * y) +
                pow(M_PI, 2) * (6 * pow(x, 3) - 13 * pow(x, 2) + 4) *
                    cos(M_PI * y) +
                8) -
           5.0 / 52.0 * M_PI *
               (x * (9 * x - 13) * cos(M_PI * y) +
           ...
           (15.0 / 364.0) * M_PI *
               ...
               cos(M_PI * x) * cos(M_PI * y) -
           5.0 / 208.0 * M_PI *
               (M_PI * x * (6 * pow(x, 2) - 23 * x + 20) * cos(M_PI * y) +
                8 * y +
                M_PI * (6 * pow(x, 3) - 13 * pow(x, 2) + 4) * sin(M_PI * y) -
                4) *
...
auto get_boundary_conditions(const MeshVariant &mv, size_t right_bound,
                             size_t left_bound) {
...
Eigen::Matrix2Xd solve(const MeshVariant &mv) {
    ...
    Eigen::Matrix2Xd nodal_forcing =
        Elasticity::integrate_volume_force(mv, force_coefficients);
    ...
    std::tie(uright, uleft) =
        get_boundary_conditions(mv, right_bound, left_bound);

    tp_start = steady_clock::now();
    auto K_unassembled = Elasticity::assemble_stiffness(mv, lambda, mu);

    Elasticity::impose_dirichlet_condition(mv, K_unassembled, nodal_forcing,
                                           right_bound, uright);
    Elasticity::impose_dirichlet_condition(mv, K_unassembled, nodal_forcing,
                                           left_bound, uleft);
    ...
    u.reshaped() =
        Eigen::SimplicialLDLT<Eigen::SparseMatrix<double>, Eigen::Upper>(K)
            .solve(nodal_forcing.reshaped());
    tp_end = steady_clock::now();
    us = duration_cast<microseconds>(tp_end - tp_start).count();
...
int main(int argc, char *argv[]) {
    ...
    for (int fi = 2; fi < argc; ++fi) {
        ...
        printf("Error for input file %s:  %E (L^2 error)  %E (L^2 u)\n",
